...
async function startWorkers(module, memory, builder) {
  ...
  _workers = await Promise.all(
    Array.from({ length: builder.numThreads() }, async () => {
      const worker = new Worker(new URL("./workerHelpers.js", "https://willcrichton.net/rust-experiments/ownership-inventory/"), {
        type: "module"
      });
...
async function init(input, maybe_memory) {
  if (typeof input === "undefined") {
    input = new URL("wasm_demo_bg.wasm", "https://willcrichton.net/rust-experiments/ownership-inventory/");
  }
  const imports = {};
  imports.wbg = {};
